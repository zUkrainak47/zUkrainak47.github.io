...
function buildSolveShareText(singleLabel, timeStr, solve) {
    return [
        `Generated by UkraTimer on ${formatReadableDate(Date.now())}`,
        '',
        `${singleLabel}: ${timeStr}${getCommentSuffix(solve)}`,
        `Scramble: ${solve.scramble}${getScrambleDateSuffix(solve)}`,
    ].join('\n');
}
...
function refreshSingleSolveSharePreview() {
    ...
    const nextShareText = buildSolveShareText(singleLabel, timeStr, previewSolve);
...
export function showSolveDetail(solve, index, isBest = null) {
    ...
    const text = buildSolveShareText(singleLabel, timeStr, solve);
